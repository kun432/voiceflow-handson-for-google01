...
「ユーザがやりたい・やってほしいと思っていること」が「インテント」、そしてそのインテントに対してのユーザの発話のバリエーションが「フレーズ」「サンプル発話」でしたね。覚えていますか？

今回は、5章で前回説明しなかった3つ目の要素、「エンティティ/スロット」を説明します。まず最初に以下の文を見てください。
...
非常に非効率ですね・・・そこでエンティティ/スロットの出番です。先程のユーザの発話、よく見ると・・・

![s402](images/s402.png)

「飲み物」のところ以外はすべて同じですよね。であれば、「飲み物」のところだけを、プログラミングで言う「変数」にして、それ以外は共通化してしまえば、メニューが増えても変数の値の種類が増えるだけだし、発話の修正やフレーズの追加なども1箇所だけで済みます。これが「エンティティ/スロット」を使うということです。とりあえず今の時点ではなんとなくイメージだけ持ってもらえばOKです。実際に使ってみればわかると思いますので、やっていきましょう。

エンティティの設定はInteraction Blockで行います。真ん中のInteraction Blockをクリックしてメニューを表示してください。

![s403](images/s403.png)

Interaction Blockのメニューの右端にある"Slots"タブをクリックしてください。（Googleでは「エンティティ」という言い方になるのですが、Voiceflowの用語はAlexa寄りなので"Slots"になってます。少しややこしいですね・・・）
...
いろいろありますね。エンティティ/スロットには「タイプ」という概念があります。例えば、”Country"タイプには、予め以下のような各国の名前が用意されています。
...
ユーザの発話を受け取ったときに、エンティティ/スロットに一致する可能性がある値のリストが予め登録されているので、発話の中のこの部分はエンティティ/スロットなんだな、ということをスマートスピーカーが認識しやすくなるというわけです。こういった「タイプ」が多数用意されているので、取りたいスロットに応じたタイプを選択することが重要になります。

![s408](images/s408.png)

予め用意されたエンティティ/スロットのタイプに適切なものがなかった場合でも、独自のタイプと独自の値のリストを定義することができます。それが「カスタムタイプ」になります。今回はメニューが予め決まっているので、それをカスタムタイプとして登録しましょう。"Custom"を選択してください。
...
はい、これで、エンティティ/スロットの設定は完了です。今度はこれをインテントから呼び出すようにします。"Intents"タブをクリックしてください。
...
フレーズ/サンプル発話を入力します。ここでエンティティ/スロットの出番です。とりあえず、"[" だけ入力してみてください。

![s418](images/s418.png)

先程設定したエンティティ名/スロット名が出てきたのがわかるでしょうか。これをクリックします。

![s419](images/s419.png)

エンティティ名/スロット名が青く網掛けされて入力されましたね。一つ半角スペースを入れて、続けて「をちょうだい」と入力してENTERをクリックします。

Negative
: スロットとそれに続くフレーズの間に必ず半角スペースを忘れないようにしてください。

![s420](images/s420.png)

このように、フレーズ/サンプル発話内にエンティティ名/スロット名を入れて入力することで、インテント・フレーズ/サンプル発話・エンティティ/スロットが紐付けられるという仕組みになっています。続けて以下も入力してください。
